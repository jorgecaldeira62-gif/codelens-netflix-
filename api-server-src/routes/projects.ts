...
router.get("/projects/:projectId/download", async (req, res): Promise<void> => {
  ...
  try {
    ...
    const skip = new Set(["node_modules", ".git", "dist", "build", ".next", ".cache", "coverage"]);

    async function addDirectory(dir: string, relative = ""): Promise<void> {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          zip.addLocalFile(fullPath, relative || undefined, entry.name);
        }
      }
    }

    await addDirectory(project.storagePath);
    const buffer = zip.toBuffer();
    const safeName = project.name.replace(/[^a-z0-9-_]+/gi, "-").replace(/^-+|-+$/g, "") || "projeto";
    res.setHeader("Content-Type", "application/zip");
    res.setHeader("Content-Disposition", `attachment; filename="${safeName}.zip"`);
    res.setHeader("Content-Length", buffer.length);
    res.send(buffer);
  } catch (err) {
    req.log.error({ err }, "Failed to create project download");
