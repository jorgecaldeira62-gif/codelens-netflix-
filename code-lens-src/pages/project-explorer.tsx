...
export default function ProjectExplorer() {
  ...
  const handleDownloadProject = useCallback(async () => {
    ...
    try {
      ...
      const resp = await fetch(`${base}/api/projects/${projectId}/download`);
      if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
      const blob = await resp.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      const name = resp.headers.get("content-disposition")?.match(/filename="([^"]+)"/)?.[1] ?? `projeto-${projectId}.zip`;
      a.download = name;
      a.click();
  ...
  return (
    <AppLayout>
      <div className="flex flex-col h-full overflow-hidden">
        <header
          className="h-12 shrink-0 flex items-center px-4 justify-between z-10"
          style={{ background: "#0a0a0a", borderBottom: "1px solid #1f1f1f" }}
        >
          <div className="flex items-center gap-3">
            <Link href="/">
              <button className="flex items-center justify-center w-8 h-8 rounded transition-colors hover:bg-white/10" style={{ color: "#888" }}>
                <ArrowLeft className="w-4 h-4" />
              </button>
            </Link>
            <div className="w-[3px] h-5 rounded-full" style={{ background: "#E50914" }} />
            <TerminalSquare className="w-4 h-4 shrink-0" style={{ color: "#E50914" }} />
            <span className="font-bold text-sm tracking-wide" style={{ color: "#e5e5e5" }}>{project.name}</span>
          </div>
          <div className="flex items-center gap-1.5">
            {[
              { label: "Terminal", icon: <Terminal className="w-3.5 h-3.5" />, onClick: () => setTerminalOpen((v) => !v) },
              { label: "Baixar", icon: isDownloading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Download className="w-3.5 h-3.5" />, onClick: handleDownloadProject, disabled: isDownloading },
              { label: "GitHub", icon: <Github className="w-3.5 h-3.5" />, onClick: () => setGithubModalOpen(true) },
            ].map(({ label, icon, onClick, disabled }) => (
              <button
                key={label}
                onClick={onClick}
                disabled={disabled}
                className="flex items-center gap-1.5 h-7 px-3 rounded text-xs font-semibold transition-all hover:opacity-90 disabled:opacity-40"
            ...
            {terminalOpen && (
              <>
                ...
                >
                  <TerminalPanel
                    projectId={projectId}
                    projectName={project.name.includes("/") ? project.name.split("/")[1] : project.name}
                    ...
                    onCommandDone={refreshProjectTree}
                    onAskAi={(text) => {
                      setExternalMessage({ text: `Analise a saída do terminal e me ajude:\n\n${text}`, id: Date.now(), contextMode: "project" });
                      // If AI panel is hidden on mobile, switch to it
                      setMobileTab("ai");
                    }}
                  />
                </ResizablePanel>
